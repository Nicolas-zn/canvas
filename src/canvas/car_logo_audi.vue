...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    // 四个圈
    ctx.beginPath()
    let startX = 400 * 0.3, offset = circleRadius / 1.7 + ctx.lineWidth
    ctx.strokeStyle = 'black'
    for (let i = 0; i < 4; i++) {
        ctx.beginPath()

        ctx.arc(startX + offset * i, canvas.height / 2, circleRadius / 2, 0, Math.PI * 2)
        ctx.stroke()
    }
...
function three_logic() {
    ...
    const plane = new PlaneGeometry(10, 5)
