...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    // 填充圆的内部
    circleRadius = circleRadius / Math.sqrt(2)
    ctx.translate(canvas.width / 2, canvas.height / 2)
    ctx.moveTo(Math.cos(Math.PI) * circleRadius, -Math.sin(Math.PI) * circleRadius)
    ctx.arc(0, 0, circleRadius, Math.PI, Math.PI * 2)
    ctx.stroke()

    ctx.translate(0, Math.cos(Math.PI / 4) * ctx.lineWidth / 2)
    ctx.moveTo(0, 0)
    ctx.lineTo(Math.cos(Math.PI / 4) * circleRadius, Math.sin(Math.PI / 4) * circleRadius)
    ctx.stroke()



    ctx.moveTo(0, 0)
    ctx.lineTo(Math.cos(Math.PI / 4 * 3) * circleRadius, Math.sin(Math.PI / 4 * 3) * circleRadius)
    ctx.stroke()

    ctx.beginPath()
    ctx.moveTo(0, 0)
    ctx.lineTo((ctx.lineWidth / 2) * Math.cos(Math.PI / 4), -(ctx.lineWidth / 2) * Math.cos(Math.PI / 4))
    ctx.lineTo(-(ctx.lineWidth / 2) * Math.cos(Math.PI / 4), -(ctx.lineWidth / 2) * Math.cos(Math.PI / 4))
    ctx.closePath()
    ctx.fill()
