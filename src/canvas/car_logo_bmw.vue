...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    let inner_circleRadius = circleRadius * 0.6
    ctx.arc(0, 0, circleRadius, 0, Math.PI * 2);
    ctx.strokeStyle = 'white'
    ctx.stroke()
    ctx.arc(0, 0, inner_circleRadius, 0, Math.PI * 2, true)
    ctx.stroke()
    ...
    ctx.beginPath()
    const startAngle = Math.PI * 1.66;
    let text = 'BMW', center_radius = (inner_circleRadius + circleRadius) / 2
    ctx.beginPath()
    const angleStep = Math.PI / 3; // 每个字符间的角度
    ctx.fillStyle = 'white'
    for (let i = 0; i < text.length; i++) {
        const char = text[i];
        const angle = startAngle + angleStep * i
        ctx.save()
        ctx.rotate(angle); // 旋转到正确角度
        ctx.translate(0, -center_radius + 10)
    ...
    ctx.fillStyle = 'rgb(55,148,245)'

    ctx.moveTo(0, 0)
    ctx.arc(0, 0, inner_circleRadius - ctx.lineWidth / 2, 0, Math.PI * 2 / 4)
    ctx.closePath()
    ctx.moveTo(0, 0)
    ctx.arc(0, 0, inner_circleRadius - ctx.lineWidth / 2, Math.PI, Math.PI * 1.5)
    ctx.closePath()
