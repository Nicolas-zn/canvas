...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    let radius = circleRadius / 3 * 2 / 2

    let right_c = [circleRadius / 2, 0]
    let center = (radius - circleRadius / 3 * 2) / 2


    ctx.beginPath()

    ctx.arc(center, 0, radius, Math.PI, 0, true)
    ctx.arc(right_c[0], right_c[1], radius, Math.PI, 0),
        ctx.arc((right_c[0] + center) / 2, 0, radius * 2, 0, Math.PI)
    ...
    ctx.arc(right_c[0], right_c[1], 25, 0, Math.PI * 2)
    ctx.fillStyle = 'white'
    ctx.fill()
}
