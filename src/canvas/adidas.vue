...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        ctx.translate(x * 25, 0)
        ctx.rotate(-Math.PI / 4 * i)
        ctx.moveTo(0, 0)
        ctx.bezierCurveTo(0, -height * golden, height * golden, -height, height, -height)
        ctx.moveTo(0, 0)
        ctx.bezierCurveTo(height * golden, 0, height, -height * golden, height, -height)
        ctx.fill()
        ctx.stroke()
        ctx.restore()
        ctx.save()
    ...
    ctx.fillStyle = 'white'
    for (let i = 0; i < 3; i++) {
        ctx.fillRect(0, canvas.height / 2 + 35 * i, canvas.width, 15)
    }
