...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    let radius = circleRadius * 0.8, inner_radius = radius * 0.45
    // 填充圆的内部
    ...
    ctx.beginPath()
    ctx.arc(0, 0, inner_radius * 0.5, 0, Math.PI * 2)
    ctx.fill()

    ctx.fillStyle = 'white'


    ctx.fillRect(-inner_radius, 0, inner_radius * 0.5, -canvas.height / 2)
}
