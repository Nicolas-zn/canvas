...
function drawSign(ctx: CanvasRenderingContext2D) {
    const start = [canvas.width * 0.1, canvas.height * 0.2]
    const end = [canvas.width * 0.8, canvas.height * 0.2]
    let center = [canvas.width * 0.45, canvas.height * 0.2]
    let cp1_1 = [center[0], center[1] * 2]
    let cen1 = [canvas.width * 0.45 * 0.2, canvas.height * 0.2]
    let cp1_2 = [cen1[0], cen1[1] * 5]
    ctx.moveTo(start[0], start[1])
    ctx.bezierCurveTo(cp1_2[0] - canvas.width * 0.05, cp1_2[1], cp1_1[0], cp1_1[1] + 10, end[0], end[1])
    let cp2_1 = [center[0], center[1] * 3]
    let cp2_2 = [cen1[0] - canvas.width * 0.25, cen1[1] * 7]
    ctx.bezierCurveTo(cp2_1[0], cp2_1[1], cp2_2[0], cp2_2[1], start[0], start[1])
    ctx.closePath()
    ctx.fillStyle = 'black'
    ctx.fill()
