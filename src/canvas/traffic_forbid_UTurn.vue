...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillRect(rectX, -rectY, rectX * 2, rectY * 3)
    let radius1 = Math.sqrt(rectX * rectX + rectY * rectY), radius2 = radius1 + rectX * 2
    ctx.beginPath()
    const angleInRadians = Math.atan(rectY / rectX);
    console.log(radius1 * Math.cos(Math.PI / 2 - angleInRadians));
    let angle = angleInRadians
    const fw = Math.PI - angle * 2
    ctx.arc(0, 0, radius1, -angleInRadians, -(fw + angleInRadians), true)
    ctx.arc(0, 0, radius2, -(fw + angleInRadians), -angleInRadians, false)
    ctx.fill()
    // return
    ctx.moveTo(rectX, -rectY);
    ctx.arc(rectX, -rectY, rectX * 2, 0, -angleInRadians, true)
    ctx.closePath()
    ctx.fill()
    ctx.moveTo(-rectX, -rectY);
    ctx.arc(-rectX, -rectY, rectX * 2, Math.PI, -(angleInRadians + fw))
    ctx.closePath()
    ctx.fill()
    ctx.fillRect(-  rectX * 3, -rectY, rectX * 2, rectY * 2)
    ...
    function drawHead() {
        ...
        let sevenCoords = [
            [-rectX * 3, rectY],
            [-rectX * 3 - x, rectY - y],
            [-rectX * 3 - x, rectY - y + height],
            [-rectX * 2, rectY + y + height],
            [-rectX + x, rectY - y + height],
            [-rectX + x, rectY - y],
            [-rectX, rectY],
        ]
