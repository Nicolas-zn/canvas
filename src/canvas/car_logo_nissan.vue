...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    ctx.beginPath();
    ctx.fillStyle = "black";
    ctx.arc(canvas.width / 2, canvas.height / 2, circleRadius * 0.75, 0, Math.PI * 2);
    ctx.arc(canvas.width / 2, canvas.height / 2, circleRadius * 0.6, 0, Math.PI * 2, true);
    ctx.closePath()
    // ctx.stroke();
    ctx.fill()
    // ctx.closePath();
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        ctx.fillText(char, circleRadius * 0.40 + offset * i, canvas.height / 2 + height * 0.2 / 2)
