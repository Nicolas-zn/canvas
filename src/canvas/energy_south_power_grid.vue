...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    ctx.lineWidth = 30
    ctx.strokeStyle = 'rgb(20,46,106)'
    let lw = 250
    // ctx.translate(canvas.width / 2,canvas.height / 2)
    let startY = 100, offset = 80
    for (let i = 0; i < 3; i++) {
        let x = canvas.width * 0.1
        let y = startY + offset * i
        ctx.moveTo(x, y)
        let ey = y + 20, ex = x + lw
        let cp1 = [x + 0.7 * lw, y + 0.15 * 20]
        let cp2 = [x + 0.95 * lw, y + 0.05 * 20]
        ctx.bezierCurveTo(cp1[0], cp1[1], cp2[0], cp2[1], ex, ey)
        ctx.lineTo(canvas.width * 0.9, ey)
        ctx.stroke()
    }

    ctx.moveTo(canvas.width * 0.1 + lw, canvas.height * 0.05)
    ctx.lineTo(canvas.width * 0.1 + lw, canvas.height * 0.8)
