...
function drawSign(ctx: CanvasRenderingContext2D) {
    ...
    const angles = [Math.PI / 6, Math.PI / 6 * 5, Math.PI / 2 * 3]
    let len = 25
    let three_coords = [[len * Math.cos(Math.PI / 6), -len * Math.sin(Math.PI / 6)], [0, len], [-len * Math.cos(Math.PI / 6), -len * Math.sin(Math.PI / 6)]]
    ctx.beginPath()
    ctx.fillStyle = 'black'
    angles.forEach((angle, index) => {
        ctx.moveTo(0, 0)
        ctx.lineTo(three_coords[index][0], three_coords[index][1])
        ctx.lineTo(circleRadius * Math.cos(angle), circleRadius * Math.sin(angle))
        if (index >= 2) {
            index = -1
            ctx.lineTo(three_coords[index + 1][0], three_coords[index + 1][1])
        } else {
            ctx.lineTo(three_coords[index + 1][0], three_coords[index + 1][1])
        }
        ctx.closePath()
